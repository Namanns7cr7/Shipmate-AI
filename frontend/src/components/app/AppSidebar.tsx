...
interface AppSidebarProps {
  ...
  onMobileClose?: () => void;
  isMobile?: boolean;
  // Mobile: also expose branch picker + actions in sidebar
...
export function AppSidebar({
  ...
  repos = [], selectedRepo = null, onSelectRepo,
  hasReport = false,
  ...
  const sidebarStyle = isMobile ? {
    ...
    display: 'flex', flexDirection: 'column' as const, padding: 16,
    overflowY: 'auto' as const,
  } : {
    width: 248, flexShrink: 0, height: '100vh', position: 'sticky' as const, top: 0,
    background: 'linear-gradient(180deg, var(--sidebar-top), var(--sidebar-bot))',
    borderRight: '1px solid var(--line)', display: 'flex', flexDirection: 'column' as const, padding: 16,
    overflowY: 'auto' as const,
  };
  ...
  return (
    <aside style={sidebarStyle}>
      ...
      {/* Mobile-only: branch picker + action buttons */}

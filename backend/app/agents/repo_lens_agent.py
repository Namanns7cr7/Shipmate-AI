...
_PY_FRAMEWORKS: Dict[str, str] = {
    ...
    "langchain": "LangChain", "boto3": "AWS SDK", "redis": "Redis",
...
def _detect_architecture_pattern(tree: List[str]) -> str:
    ...
    # Fullstack — explicit frontend + backend split
    if _has_top_level_dir(tree, "frontend") and _has_top_level_dir(tree, "backend"):
        return "fullstack"
...
class RepoLensAgent(BaseAgent):
    ...
    def _detect_stack(self, tree: List[str], kf: Dict[str, str]) -> List[str]:
        ...
        if any(
            Path(f).name.startswith("vite.config") for f in tree
        ):
            stack.add("Vite")
        if any(
            Path(f).name.startswith("next.config") for f in tree
        ):
            # Next.js already in npm dep scan; but catch projects missing package.json read
            stack.add("Next.js")
        if any(
            Path(f).name.startswith(("tailwind.config", ".tailwindrc")) for f in tree
        ):
        ...
        if any("dockerfile" in f.lower() for f in tree):
            stack.add("Docker")
        if any(
            Path(f).name in ("docker-compose.yml", "docker-compose.yaml") for f in tree
        ):
    ...
    def _assess_risks(
        ...
        has_lint = any(
            any(m in f for m in _LINT_MARKERS)
            for f in tree
        )
        if not has_lint:
            risks.append(ArchitectureRisk(
